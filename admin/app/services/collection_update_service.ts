...
export class CollectionUpdateService {
  async checkForUpdates(): Promise<ContentUpdateCheckResult> {
    const babylonAPIURL = env.get('BABYLON_API_URL') || BABYLON_API_DEFAULT_BASE_URL
    if (!babylonAPIURL) {
      return {
        updates: [],
        checked_at: new Date().toISOString(),
        error: 'Babylon API is not configured. Set the BABYLON_API_URL environment variable.',
      }
    }
    ...
    try {
      const response = await axios.post<ResourceUpdateInfo[]>(`${babylonAPIURL}/api/v1/resources/check-updates`, requestBody, {
        timeout: 15000,
      })
      ...
      if (axios.isAxiosError(error) && error.response) {
        logger.error(
          `[CollectionUpdateService] Babylon API returned ${error.response.status}: ${JSON.stringify(error.response.data)}`
        )
        return {
          updates: [],
          checked_at: new Date().toISOString(),
          error: `Babylon API returned status ${error.response.status}`,
        }
      }
      const message =
        error instanceof Error ? error.message : 'Unknown error contacting Babylon API'
      logger.error(`[CollectionUpdateService] Failed to check for updates: ${message}`)
      return {
        updates: [],
        checked_at: new Date().toISOString(),
        error: `Failed to contact Babylon API: ${message}`,
      }
    }
  }
